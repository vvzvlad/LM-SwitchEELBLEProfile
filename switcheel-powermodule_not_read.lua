return {
  ...
  read = function(device)
    ...
    if (err ~= nil and err ~= 'failed to bind socket') then log('Read connect to '..device.name..'('..device.id..')'..' return error: '..err..'(use socket '..sock..')') end
    if (err == 'failed to bind socket') then log('Read connect to '..device.name..'('..device.id..')'..' return error: '..err..'(maybe device power-off?)') end
    if (res == true) then
      local i = 0
      while (status == false and i<10) do
        if (i == 2) then
          ...
          if (err ~= nil) then log('Read second connect to '..device.name..'('..device.id..')'..' return error: '..err..'(use socket '..sock..')') end
        end 
        ...
        if (#value_channel == 7) then
          ...
          values.read_correct = false
          status = false
        end

        local switcheel_keys = storage.get("switcheel_keys") or {}
        if (#switcheel_keys[device.id] == 16) then
          values.key_saved = true
        else
          values.key_saved = false
        end
